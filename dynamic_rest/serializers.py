...
class WithDynamicSerializerMixin(object):
    ...
    def __init__(
        ...
        self.request_fields = request_fields or self._context.get(
            'request_fields',
            {})
        self.only_fields = only_fields or self._context.get('only_fields', [])
        include_fields = include_fields or self._context.get(
            'include_fields',
            [])
        exclude_fields = exclude_fields or self._context.get(
            'exclude_fields',
            [])
    ...
    def get_fields(self):
        ...
        if only_fields:
            return {
                k: v for k, v in serializer_fields.iteritems() if k in only_fields}
        ...
        deferred = set(
            [name for name, field in serializer_fields.iteritems()
             if getattr(field, 'deferred', None) == True or name in
             meta_deferred])
